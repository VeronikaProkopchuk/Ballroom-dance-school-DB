...
from .model import LeccionesModel

class WindowLecciones(QMainWindow):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Lecciones")
        self.resize(550, 250)
        self.centralWidget = QWidget()
        self.setCentralWidget(self.centralWidget)
        self.layout = QHBoxLayout()
        self.centralWidget.setLayout(self.layout)
        self.leccionesModel = LeccionesModel()
        self.setupUI()

    def setupUI(self):
        # Crear el widget de la tabla
        self.table = QTableView()
        self.table.setModel(self.leccionesModel.model)
    ...
    def openAddDialog(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.leccionesModel.addLeccion(dialog.data)
            self.table.resizeColumnsToContents()
    ...
    def deleteContact(self):
        ...
        messageBox = QMessageBox.warning(
            self,
            "Cuidado!",
            "¿Seguro que quiere eliminar la leccion?",
            QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel,
        )

        if messageBox == QMessageBox.StandardButton.Ok:
            self.leccionesModel.deleteLeccion(row)

    def clearContacts(self):
        messageBox = QMessageBox.warning(
            self,
            "Cuidado!",
            "Seguro que quiere eliminar todos las lecciones?",
            QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel,
        )

        if messageBox == QMessageBox.StandardButton.Ok:
            self.leccionesModel.clearLecciones()
