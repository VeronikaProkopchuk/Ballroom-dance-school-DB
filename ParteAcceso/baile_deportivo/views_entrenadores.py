...
from .model import EntrenadoresModel

class WindowEntrenadores(QMainWindow):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Entrenadores")
        self.resize(550, 250)
        self.centralWidget = QWidget()
        self.setCentralWidget(self.centralWidget)
        self.layout = QHBoxLayout()
        self.centralWidget.setLayout(self.layout)
        self.entrenadoresModel = EntrenadoresModel()
        self.setupUI()

    def setupUI(self):
        # Crear el widget de la tabla
        self.table = QTableView()
        self.table.setModel(self.entrenadoresModel.model)
    ...
    def openAddDialog(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.entrenadoresModel.addEntrenador(dialog.data)
            self.table.resizeColumnsToContents()
    ...
    def deleteContact(self):
        ...
        messageBox = QMessageBox.warning(
            self,
            "Cuidado!",
            "¿Seguro que quiere eliminar el entrenador?",
            QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel,
        )

        if messageBox == QMessageBox.StandardButton.Ok:
            self.entrenadoresModel.deleteEntrenador(row)

    def clearContacts(self):
        messageBox = QMessageBox.warning(
            self,
            "Cuidado!",
            "Seguro que quiere eliminar todos los entrenadores?",
            QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel,
        )

        if messageBox == QMessageBox.StandardButton.Ok:
            self.entrenadoresModel.clearEntrenadores()
